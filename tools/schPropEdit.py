...
REQUIRED_CSV_COLUMNS = ('search_name', 'search_value', 'change_name', 'change_value')
...
def load_replacements_from_csv(path):
    "Read the replacement table and return a list of (search_name, search_value, change_name, change_value)"
    replacements = []
    with open(path, newline='', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        missing_columns = set(REQUIRED_CSV_COLUMNS) - set(reader.fieldnames or [])
        if missing_columns:
            logging.error(f"CSV is missing columns: {', '.join(sorted(missing_columns))}")
            sys.exit(2)
        for row_number, row in enumerate(reader, start=2):
            values = tuple((row.get(column) or '').strip() for column in REQUIRED_CSV_COLUMNS)
            if not all(values):
...
for search_name, search_value, change_name, change_value in replacements:
    kicad_changer.change_property(search_name, search_value, change_name, change_value)
